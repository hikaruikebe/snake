...
class Agent:
    def __init__(self):
        ...
        self.memory = deque(maxlen=MAX_MEMORY)  # popleft() if max length
    ...
    def train_long_memory(self):
        if len(self.memory) > BATCH_SIZE:
            # list of tuples of random samples
            mini_sample = random.sample(self.memory, BATCH_SIZE)
        else:
            mini_sample = self.memory

        # combine states, actions, rewards, next_states, gameovers
...
def train():
    ...
    while True:
        ...
        reward, gameover, score, gameover_reason = game.play_step(final_move, time)
        state_new = agent.get_state(game)
        time += 1

        if gameover_reason == 1:
            cause = "collision"
        elif gameover_reason == 2:
            cause = "lost"
        ...
        if gameover:
            ...
            # moving average
            if agent.n_games > 10:
                moving_mean_score = sum(plot_scores[len(plot_scores) - 10 :]) / 10
            else:
                moving_mean_score = sum(plot_scores) / agent.n_games
            plot_moving_mean_scores.append(moving_mean_score)
            ...
            plot(plot_scores, plot_mean_scores, plot_moving_mean_scores)
