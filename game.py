...
class SnakeGameAI:
    ...
    def play_step(self, action, time):
        ...
        if self.is_collision() or self.frame_iteration > 100 * len(self.snake):
            ...
            if self.frame_iteration > 100 * len(self.snake):
                return reward, game_over, self.score, 2
            return reward, game_over, self.score, 1

        # 4. place new food or just move
        if self.head == self.food:
            self.score += 1
            reward = 10  # + 100 * len(self.snake) / self.frame_iteration
            self._place_food()
        else:
            self.snake.pop()
